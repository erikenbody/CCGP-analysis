...
get_input_objects <- function(species, data_path, analysis = "gendist", pruned = TRUE, impute = "none", kvals = NULL, rmislands = TRUE) {
  
  # Get coords --------------------------------------------------------------
  coords <- readr::read_tsv(paste0(data_path, species, "/", species, ".coords.txt", sep = ""), col_names = FALSE)
  colnames(coords) <- c("INDV", "x", "y")
  
  # Get input data ----------------------------------------------------------
  if (analysis == "gendist") {
    if (!pruned) {
      dist = paste0(data_path, species, "/", species, "_filtered.dist", sep = "")
      dist_id = paste0(data_path, species, "/", species, "_filtered.dist.id", sep = "")
    }
    if (pruned) {
      dist = paste0(data_path, species, "/", species, "_annotated_pruned_0.6.dist", sep = "")
      dist_id = paste0(data_path, species, "/", species, "_annotated_pruned_0.6.dist.id", sep = "")
    }
  ...
  if (analysis == "vcf") {
    if (!pruned) {
      gen <- vcfR::read.vcfR(paste0(data_path, species, "/", species, "_annotated.vcf.gz"))
    }
    if (pruned) {
      gen <- vcfR::read.vcfR(paste0(data_path, species, "/", species, "_annotated_pruned_0.6.vcf.gz"))
    }
    if (impute == "simple") gen <- algatr::simple_impute(gen = gen, FUN = median)
  ...
  # Get envlayers -----------------------------------------------------------
  env <- get_envlayers(env_path = "data/", rmislands = rmislands)
